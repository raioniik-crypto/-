...
export default function Mascot({ message, className = "", size = "md" }: MascotProps) {
  ...
  return (
    <div className={`relative flex items-end gap-4 ${className}`}>
      <AnimatePresence>
        {message && (
          ...
          >
            ...
            <div className="absolute -bottom-4 left-10 w-0 h-0 border-l-[10px] border-l-transparent border-r-[10px] border-r-transparent border-t-[10px] border-t-black" />
            <div className="absolute -bottom-2 left-[42px] w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-t-[8px] border-t-white" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        <div className="absolute inset-x-2 -bottom-2 h-4 bg-black/10 rounded-full blur-md" />
        <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-[4px_4px_0px_rgba(0,0,0,1)]">
          ...
          <motion.g animate={{ opacity: [0, 1, 0], scale: [0.5, 1, 0.5] }} transition={{ duration: 2, repeat: Infinity }}>
